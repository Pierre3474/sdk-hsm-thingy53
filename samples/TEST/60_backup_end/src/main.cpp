...
const struct device *sensor_dev_bmi270;
...
void app_bmi270_init() {
    sensor_dev_bmi270 = DEVICE_DT_GET_ONE(bosch_bmi270);
    if (!device_is_ready(sensor_dev_bmi270)) {
        printk("Sensor device bmi270 not ready\n");
        return;
    }
}
...
void bmi270_get_values(int32_t &accel_x, int32_t &accel_y, int32_t &accel_z,
                       int32_t &gyro_x, int32_t &gyro_y, int32_t &gyro_z) {
    int ret;
    struct sensor_value accel[3], gyro[3];

    ret = sensor_sample_fetch(sensor_dev_bmi270);
    if (ret) {
        printk("sensor_sample_fetch failed ret %d\n", ret);
        return;
    }

    ret = sensor_channel_get(sensor_dev_bmi270, SENSOR_CHAN_ACCEL_XYZ, accel);
    if (ret) {
        printk("sensor_channel_get accel failed ret %d\n", ret);
        return;
    }
    accel_x = accel[0].val1;
    accel_y = accel[1].val1;
    accel_z = accel[2].val1;

    ret = sensor_channel_get(sensor_dev_bmi270, SENSOR_CHAN_GYRO_XYZ, gyro);
    if (ret) {
        printk("sensor_channel_get gyro failed ret %d\n", ret);
        return;
    }
    gyro_x = gyro[0].val1;
    gyro_y = gyro[1].val1;
...
int main(void)
{
	...
	while(1){
		...
		int32_t accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z;
		bmi270_get_values(accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z);
		...
		data["gyro_y"] = gyro_y;
		data["gyro_z"] = gyro_z;
		topic = "thread_tags/"+std::string(uid_text)+"/imu";
		send_data(topic.c_str(), data);
		data = {};
	}
}
